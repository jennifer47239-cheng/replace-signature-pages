...
| **C+ · 分组包** | 抽出后按签署主体 / 签字人分包 | `*_签字页分组包/` + ZIP |

**全程本机、无网络、不调用 AI**，适合处理未脱敏合同。

当前版本：**0.7.3**（标签工作台：标签库 + 按页分配、缩略图放大、草稿保存；详见 [CHANGELOG.md](./CHANGELOG.md)）
...
# 流程 C+：按投资方 / 签字人分组（标签 JSON，本机手填权威）
...
流程 C+：确认页码后按 **签署主体**（投资方或融资方等）/ **签字人** 标签分组。产物：`按签署主体|按签字人/` + ZIP。

标签工作台（本机 localhost 页面）分两步：**① 标签库**——先把签署主体 / 签字人 / 身份整理确认好（可点本机扫描候选、也可手填）；**② 分配页码**——每行选一页（下拉含整段与每一页）再选一个标签，页码可用行内 ◀ ▶ 翻，左侧缩略图会跟着高亮；行数多时用「批量加行」（一个标签 × 多页）或「按页拆行」（每页一行）。

候选标签区采用固定高度并可独立滚动。标签整理到一半可点「保存标签草稿」；
草稿仅写入本机工作目录，刷新当前页面会自动恢复标签库、页码分配和当前步骤。

> 每行的页码决定该标签拿到哪些页。若多行都留着同一个多页整段，每个分组 PDF 都会包含整段全部页，看起来就像「没分组」。
...
启动后可选：**嵌回电子版** / **双面打印包** / **仅提取签字页** / **签字页分组包** / **批量打印包**。
